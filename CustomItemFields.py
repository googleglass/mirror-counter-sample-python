...
class CustomItemFields(object):
    ...
    DEFAULT_TEMPLATE_URL = 'templates/item.html'
    ...
    def get_fields_from_item(cls, item):
        """Constructs a new CustomitemFields object from json_input assuming that input 
	    JSON is in item['sourceItemId']"""
        key = 'sourceItemId'
        if key in item:
    ...
    def render_html(self):
        """Returns html rendering of timeline item with self._fields data. Returns
	empty string if object does not have a template."""
	if self.template:
